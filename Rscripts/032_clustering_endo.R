...
analysis_step <- "032_clustering_endo"
...
features <- readLines("aux_data/gene_set/030_str.txt")
...
### c1, c15 & c16: heterotypic multiplet with Myocytes, c11: prolif, c18: contamination, c19: LEC?? LEC seem not clustering well...
### >>> decided to remove c1, c15, c16, c18 & c19

# ITER2: remove probable contamination, then re-cluster ----
plot_path <- file.path(plot_path_root, "iter2")
fs::dir_create(c(plot_path))
seu <- subset(seu, idents = c(1,15,16,18,19), invert = TRUE)
seu[["RNA"]] <- split(seu[["RNA"]], f = seu$orig.ident)
